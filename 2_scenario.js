...
async function readAddressesFile() {
    try {
      ...
      const addressModifyArray = addressArray.map(address => {
        ...
        return sql`EXPLAIN ANALYZE SELECT object_id
        FROM objects
        WHERE(owner_type = 'address_owner' and owner_address = ${address} and object_status NOT IN ('deleted', 'wrapped', 'unwrapped_then_deleted'))
        LIMIT 20;`;
      });
